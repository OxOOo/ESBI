	  ...
	  flow_t *Flow, int *FlowSize)
{
  ...
  node2_t *XP;
  flow_t *FlowP;
  static node1_t U[MAX_SIG_SIZE1], V[MAX_SIG_SIZE1];
		  ...
		  double (*Dist)(feature_t *, feature_t *))
{
  ...
  feature_t *P1, *P2;
  static double S[MAX_SIG_SIZE1], D[MAX_SIG_SIZE1];
...
static void newSol()
{
    ...
    double xMin;
    int steps;
    static node2_t *Loop[2*MAX_SIG_SIZE1], *CurX, *LeaveX;
...
static int findLoop(node2_t **Loop)
{
  int i, steps;
  node2_t **CurX, *NewX;
  static char IsUsed[2*MAX_SIG_SIZE1]; 
...
static void russel(double *S, double *D)
{
  ...
  static node1_t Ur[MAX_SIG_SIZE1], Vr[MAX_SIG_SIZE1];
